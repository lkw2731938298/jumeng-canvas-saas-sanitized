...
[切段清单 / 各段参考切片]
        │
        │  storyboard_overseas_localize（目标市场）
        ▼
[本地化后的切段文案与主体]
        │
        │  问过之后才出
...
[主体图 image_input × 主体数]（用户跳过则没有）
        ...
        │  再问过、用户同意之后
        ▼
[出海段1 video_input] … [出海段N video_input]
        │
        │  全部成功后系统拼接
        ▼
[一条成片] → 资产 + 对话框
...
| 切段清单 | `storyboard_grid` | 切段·出海 | `storyboard_from_video` 写入，再经本地化；对用户不当成要去编辑的表 |
| 主体图 | `image_input` | 主体·{短名} | 仅用户同意出图后才建/生成；外形符合目标市场 |
| 出海段 i | `video_input` | {市场}段{i} | 整数时长 5–12；连本段切片 + 本段主体图 |
...
| 出海段 i | `(ax + 860, ay + (i-1)*220)` |

## 3. 落点顺序

### 参考与市场

有附件 → `add_node` `video_input` + `assetId`。已有带媒体的参考 → 复用。市场未定则先 `ask_user`。
...
`run_canvas_tool` `storyboard_overseas_localize`，目标为切段清单，带上市场 id。
...
本轮生成一个批次 id（例如 `overseas-` + 时间戳），**所有出海段共用**。

每个 `add_node` `video_input`：

- `prompt`：该段画面 + 运镜 + 本地化对白/主体描述；主体短名与主体图一致
...
- 该段参考**切片**（切段后的片段节点或表内参考）→ 该出海段 `ref_in`
- 本段出场的主体图 → 同一 `ref_in`
- 对照本轮模型参考路数上限；超了只留本段最必要的主体图 + 切片
- 不要把完整参考原片连到出海段上当生成目标
...
| 切段后本地化 | 一次 `storyboard_overseas_localize` |
| 用户同意出主体图 | 抽主体 + 出图（不要夹带视频 generate） |
| 用户同意出视频 | 多个 add 出海段 + connect + 全部 generate_node |
