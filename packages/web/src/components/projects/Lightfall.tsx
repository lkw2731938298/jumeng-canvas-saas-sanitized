...
const Lightfall: React.FC<LightfallProps> = ({
  ...
  streakLength=1, glow=1, density=0.6, twinkle=1, zoom=3,
  backgroundGlow=0.5, opacity=1, mouseInteraction=true,
  mouseStrength=0.5, mouseRadius=1, mouseDampening=0.15, mixBlendMode
}) => {
  ...
  useEffect(() => {
    ...
    const uniforms:Record<string,{value:unknown}>={
      ...
      uGlow:{value:glow},uDensity:{value:density},uTwinkle:{value:twinkle},uZoom:{value:zoom},
      uBgGlow:{value:backgroundGlow},uOpacity:{value:opacity},
      uMouseEnabled:{value:mouseInteraction?1:0},uMouseStrength:{value:mouseStrength},uMouseRadius:{value:mouseRadius}
    };
    const program=new Program(gl,{vertex,fragment,uniforms}); programRef.current=program;
    const geometry=new Triangle(gl); geometryRef.current=geometry;
    const mesh=new Mesh(gl,{geometry,program}); meshRef.current=mesh;
    const resize=()=>{const rect=container.getBoundingClientRect();renderer.setSize(rect.width,rect.height);(uniforms.iResolution.value as number[])[0]=gl.drawingBufferWidth;(uniforms.iResolution.value as number[])[1]=gl.drawingBufferHeight;};
    resize();
    const ro=new ResizeObserver(resize);ro.observe(container);
    const onPointerMove=(e:PointerEvent)=>{const rect=canvas.getBoundingClientRect();const scale=renderer.dpr||1;const x=(e.clientX-rect.left)*scale;const y=(rect.height-(e.clientY-rect.top))*scale;mouseTargetRef.current=[x,y];};
  ...
  return <div ref={containerRef} className={className} style={{...mixBlendMode&&{mixBlendMode: mixBlendMode as React.CSSProperties['mixBlendMode']},width:'100%',height:'100%',overflow:'hidden',position:'relative'}}/>;
};
